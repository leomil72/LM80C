...
INCTMR3:        inc (hl)            ; increment timer
                ...
                jr nz,EXCH3T        ; if not, then exit (finished increment)
                inc hl              ; there was an overflow, so increment next byte
                djnz INCTMR3        ; repeat for max 4 bytes
EXCH3T:         pop hl              ; recover regs. A,
                pop bc              ; BC,
                pop af              ; and HL
                ei                  ; re-enable interrupts
                reti                ; exit from ISR
...
INIT_HW:        ; first run - setup HW & SW
                ...
                ld de,$0001             ; E chooses the video mode:
...
;f = CPU_CLK/(144*256) => 3,686,400 / ( 36,864 ) => 100Hz
                ...
                out (CTC_CH3),a
                ld a,0x90           ; time constant - 90$ stands for 144d
                out (CTC_CH3),a
...
initVDP:        ; initialize VDP for a specific graphics mode
                ...
                ld b,$08            ; 8 registers means 8 bytes
                push de             ; store reg. E
                sla e               ; multiply by 8
...
EMPTYVRAM:      out (VDP_RAM),a     ; after first byte, the VDP autoincrements VRAM pointer
                ...
                ; set VDP to required mode
                pop de              ; recover reg. E
                ld a,e              ; move E into A
                cp $01
                jp z,G1MD          ; jump to graphics 1 mode
                cp $02
                jp z,G2MD          ; jump to graphics 2 mode
                cp $03
                jp z,MCMD          ; jump to multicolor mode
                ; if $00 or no cases are valid, set to text mode
; TEXT MODE
TXTMD:          ; load charset
                ld b,$7f            ; 127 chars to be loaded
                ld hl,$4000         ; first pattern cell $0000 (+$4000 bcs. MSB must be 0 & 1)
                call LOADCHARSET    ; load patterns into VRAM
                ; welcome message
                ld hl,$4800         ; position at X,Y=0,0 in name table (+$4000 bcs. MSB must be 0 & 1)
...
G1MD:           ; load pattern table
                ld b,$7f            ; 127 patterns to be loaded (same as for text mode)
                ld hl,$4000         ; first pattern cell $0000 (+$4000 bcs. MSB must be 0 & 1)
                call LOADCHARSET    ; load patterns into VRAM
...
LDCLRTBMD1:     out (VDP_RAM),a     ; after first byte, the VDP autoincrements VRAM pointer
                nop
                djnz LDCLRTBMD1     ; repeat for 32 bytes
                ; load welcome message into name table
                ld hl,$5800         ; position at X,Y=0,0 in name table, that is $1800 (+$4000 bcs. MSBs must be 0 & 1, resp.)
                ld de,WLCMSG        ; load start address of welcome message
                call WELCOMEMSG     ; print welcome message
...
WELCOMEMSG:     ; reg. DE contains address of message
                ; reg. HL contains address of table address
                ld c,VDP_REG        ; load VPD port value
                out (c),l           ; low byte of address to VDP
                out (c),h           ; high byte address to VDP
...
VDPMODESETMC    ; VDP register settings for a multicolor mode
                ...
                defb $05        ; reg.2: name table addr.: $1400
                defb $00        ; reg.3: don't care
                defb $01        ; reg.4: pattern table addr.: $0800
...
initSPRITES:    ld c,VDP_RAM        ; load VDP port into C
                ...
                ld de,$7800         ; load sprite pattern address ($4000+$0000)
                ld b,$20            ; 20 bytes for a 16x16 sprite
                out (c),e           ; send address to VDP
                out (c),d           ; to first cell to write to
                otir                ; read from (HL), send to (C), then decrement B - repeat until B = 0
                ld hl,SPRTATTRDATA  ; load first cell of sprite patterns
                ld de,$5000         ; load sprite attr. address ($4000+$1000)
                ld b,$4             ; 4 bytes for a sprite
                out (c),e           ; send address to VDP
                out (c),d           ; to first cell to write to
                otir                ; read from (HL), send to (C), then decrement B - repeat until B = 0
                ret

SPRITEDEMODATA: defw $0103, $0303, $0103, $0305 ; sprite 00
                defw $0F03, $0307, $070E, $0C06
                defw $C0A0, $E0C0, $80C0, $F0F8
                defw $C0C0, $F070, $6030, $0000
                defw $0103, $0303, $0103, $0307 ; sprite 04
                defw $0303, $0307, $0E0C, $0800
                defw $C0A0, $E0C0, $80C0, $E0A0
                defw $C0C0, $C0C0, $C0C0, $C060
                defw $0103, $0303, $0103, $0305 ; sprite 08
                defw $0503, $0307, $0503, $0303
                defw $C0A0, $E0C0, $80C0, $C0A0
                defw $A0C0, $E0E0, $8080, $0080
SPRTATTRDATA:   defb $20, $20, $00, $04         ; sprite 00: X,Y=0,0 color = dark blue, pattern=00
;------------------------------------------------------------------------------
...
PBUFF   equ    WRKSPC+0EDH          ; Number print buffer
MULVAL  equ    WRKSPC+0FAH          ; Multiplier
PROGST  equ    WRKSPC+0FDH          ; Start of program text area
STLOOK  equ    WRKSPC+161H          ; Start of memory test
...
; The folloeing list reports all the functions supported by the interpreter.
; To add a custom function, the user must firt insert the reserved word here,
; then in the list of the reserved words below, and finally must increment the
...
FNCTAB: defw   SGN      ; this list must be coherent with the tokens' functions list
...
WORDTB: defw   PEND
        ...
        defw   POKE
        defw   DOKE
        defw   REM          ; not implemented (was SCREEN)
        defw   LINES
        defw   CLS
        defw   WIDTH
        defw   MONITR
