...
;------------------------------------------------------------------------------
; interrupt vector for CH2 Timer - used for 100ths/s counter
                org $14
                defw CH2_TIMER
...
INCTMR2:        inc (hl)            ; increment timer
                cp (hl)             ; compare (hl) with A (0)
                jr nz,EXCH2T        ; if not, then exit (finished increment)
                inc hl              ; there was an overflow, so increment next byte
                djnz INCTMR2        ; repeat for max 4 bytes
EXCH2T:         pop hl
                pop bc
