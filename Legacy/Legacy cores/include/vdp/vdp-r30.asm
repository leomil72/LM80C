...
G1MD:           ; load pattern table
                ...
                call    SETVDPADRS
                ld      A,$0F           ; foreground color...
                ld      (FRGNDCLR),A    ; ...set to white
                ld      A,$05           ; background color...
                ld      (BKGNDCLR),A    ; ...set to light blue
...
SETGRPVOID:     ld      A,(SCR_SIZE_H)  ; load height of screen
...
SENDCR:         ld      A,CR            ; yes, so now send carriage return
                call    CHARINTOBFR     ; send to buffer
                ;---    final part: go at the beginning of the new on the screen
...
SET_GFX_MODE2:  ld      HL,VDPMODESET   ; pointer to register settings | <= here points the SHOW_LOGO sub-routine
