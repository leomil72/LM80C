...
SETTOP: call    CURSOR_OFF      ; disable cursor
        ...
        ld      (IOBUFF),HL     ; ...and store address
        ld      DE,0-32         ; then, reserve 32 bytes...
        add     HL,DE           ; ...for DOS buffer...
        ld      (DOSBFR),HL     ; ...and store address
        ld      DE,0-1          ; then, 1 more byte to go back to preserve...
        add     HL,DE           ; ... the 1st byte of buffer
NODOS:  ld      DE,0-100        ; now, preserve 100 bytes for string space
...
SIGNON: defb    "LM80C BASIC 3.19 ",251,"2021 L.Miliani"
...
WORDTB: defw    PEND
        ...
        defw    DISK        ; added by Leonardo Miliani
        defw    WAIT
        defw    DEF
        defw    POKE
        defw    DOKE
...
ZPLUS   equ     $BE             ; +         <-- from here, there are the math operators
ZMINUS  equ     $BF             ; -
ZTIMES  equ     $C0             ; *
ZDIV    equ     $C1             ; /
ZMOD    equ     $C2             ; %
...
ZSGN    equ     $CB             ; SGN       <-- from here, there are the functions
ZPOINT  equ     $E1             ; ZPOINT    <-- if the user enters a custom function, between
                                ;               SGN and POINT, he/she must increment this pointer by 1
ZINSTR  equ     $E2             ; ZINSTR    <-- same here
ZLEFT   equ     $EA             ; LEFT$     <-- if the user enters a custom function anywhere,
...
; check if DOS is enabled
CHKDOS: ld      A,(DOS_EN)      ; load DOS state
        and     A               ; check if 0 (NOT enabled)
        ret     NZ              ; return if enabled
        ld      E,D0            ; DOS not enabled error
        jp      ERROR           ; raise error


; LOAD "filename"[,1]  <- load a BASIC program from disk
; LOAD x,y,z           <- load a sector from disc, LBA address equals to (((z*256)+y)*256)+x
LOAD:   call    LDSVPT          ; check if in direct mode and what's following
...
LOADST: ld      HL,(TMPBFR1)    ; retrieve HL
        call    GET3AR          ; get 3 arguments & set corresponding sector
        ld      (TMPBFR1),HL    ; store HL
...
LDSVPT: call    CHKDOS          ; check if DOS is enabled
        dec     HL              ; dec 'cos GETCHR INCs
        call    GETCHR          ; check if something follows
...
; get 3 arguments after the command LOAD or SAVE
; and set CF to point to the selected sector
GET3AR: ld      HL,(TMPBFR1)     ; retrieve string code pointer
...
SAVE:   call    CHKDOS          ; check if DOS is enabled
        dec     HL              ; dec 'cos GETCHR INCs
        call    GETCHR          ; check if something follows
        jp      Z,SNERR         ; if nothing else, raise a syntax error
        ld      (TMPBFR1),HL    ; save current code string pointer
        ...
        ld      A,(TYPE)        ; Get variable type
        or      A               ; Is it a string variable?
        jp      Z,SAVEST        ; no - load a single sector
        call    CHKFN1          ; yes, evaluate string
        ld      A,$80           ; set BAS for file type
        ld      (TPBF4),A
        dec     HL              ; dec 'cos GETCHR INCs
        call    GETCHR          ; check if something follows
...
SAVEST: ld      HL,(TMPDBF)     ; retrieve string code pointer
        call    GET3AR          ; get 3 arguments
        call    CLRIOBF         ; clear I/O buffer
        call    CF_INIT         ; init CF card
        jp      C,NRERR         ; error if device not available/ready
        push    HL              ; store code string pointer
        ...
        call    CF_STANDBY      ; put CF in standby mode
        xor     A               ; clear Carry
SVEND:  pop     HL              ; retrieve code string pointer
        jp      C,DOS_ERR       ; jump if DOS error
        ret                     ; return to caller
TXTSVG: defb    "Saving file... ",CR,0


; FILES
; list files on disk
FILES:  call    CHKDOS          ; check if DOS is enabled
        call    CLRIOBF         ; clear I/O buffer
        call    CF_INIT         ; init CF card
...
ERASE:  call    CHKDOS          ; check if DOS is enabled
        call    CHKFLNM         ; check for a disk name
        call    CHK1AR          ; check for ",1" argument
        push    HL              ; store code string pointer
...
DISK:   call    DIRMOD          ; can be executed ONLY in direct mode
        jp      NZ,IMERR        ; raise error if in indirect mode
        call    CHKDOS          ; check if DOS is enabled
        ...
        and     %01011111       ; Force upper case
        cp      'F'             ; format command?
        jp      Z,DSKFRM        ; yes, format disk
        cp      'R'             ; rename command?
        jp      Z,DSKRNM        ; yes, rename disk
        jp      SNERR           ; raise a syntax error


; Format disk or rewrite Master sector
; syntax: DISK "F","disk name"[,1]
; "disk name" is the name -> max 16 chars, allowed chars: "A" to "Z", "0" to "9","-", SPACE
; ",1": just rewrite Master sector without overwriting current directory
...
DSKFRM: call    CHKSYN          ; Make sure ',' follows
        ...
        call    CHK1AR          ; check if ",1" follows - saved into A
        ld      DE,MSTTXT       ; format message
        or      A               ; A=0?
        jr      NZ,CFINIT       ; no, jump over
        ld      DE,FRMTXT       ; only Master Sector
...
INITDSK:call    CF_INIT         ; init CF card
        jp      C,DOS_ERR       ; error if device not available/ready
        push    BC
        push    DE
        push    HL              ; store code string pointer
        ...
        pop     DE
        pop     BC
        jr      NC,INITFN       ; leave if no errors
        jp      DOS_ERR         ; raise error
INITFN: push    HL              ; store code string pointer
        ld      A,(TMPBFR1)
        or      A               ; full formatting?
        jr      NZ,MSPTOK       ; no
        ld      HL,FRMTOK       ; yes, Point to message
        call    PRS             ; print message for init confirmation
        push    DE
        ...
        pop     HL
        ret                     ; return to caller
MSPTOK: ld      HL,MSWROK       ; format OK message
        call    PRS             ; print message for init confirmation
        pop     HL              ; retrieve code string pointer
        ret                     ; return to caller
MSTTXT: defb    "Rewrite Master Sector? ",0
FRMTXT: defb    "WARNING!! Disk contents will be erased. "      ; the spacing is calculated for both 32&40 cols video modes.
        defb    "Proceed? ",0                                   ; do not add other chars  
FRMTOK: defb    CR,"Formatting completed",CR,0
MSWROK: defb    CR,"Master Sector rewritten",CR,0
...
CHKFN1: call    TSTSTR          ; Make sure it's a string
        ...
        ld      DE,(TMPDBF+4)   ; retrieve DE
        ld      BC,(TMPDBF+6)   ; retrieve BC
        ret                     ; return to caller
...
RESET:  call    DISNMI          ; disable NMI vector
        ...
        ld      A,(DOS_EN)      ; is DOS enabled
        and     A               ; if enabled, A<>0
        call    NZ,CF_STANDBY   ; put CF into standby mode
        di                      ; disable INTs
    IFDEF LM80C64K
        jp      ROM2RAM         ; Restart
    ELSE
