...
SCR_NAM_TB  equ     WRKSPC+0F1H         ; (2) screen name table address
SCR_CURS_X  equ     WRKSPC+0F3H         ; (1) cursor X position
SCR_CURS_Y  equ     WRKSPC+0F4H         ; (1) cursor Y position
...
CURSOR_ON   equ     WRKSPC+0F8H         ; (1) state of cursor (1=on, 0=off)
LSTCSRSTA   equ     WRKSPC+0F9H         ; (1) last cursor state
BUFVIDEO    equ     WRKSPC+0FAH         ; (1) buffer video on/off
CHR4VID     equ     WRKSPC+0FBH         ; (1) char for video buffer
PBUFF       equ     WRKSPC+0FCH         ; Number print buffer
...
;INIT:   ld      HL,INITAB           ; Initialize Workspace
;        ld      BC,INITBE-INITAB+3  ; Bytes to copy
;        ld      DE,WRKSPC           ; Into workspace RAM
;        ldir                        ; Move bytes until done (BC=0)
...
ZLEFT   equ    0CEH +2         ; LEFT$      ; he/she must be increment these two pointers
