...
; BY STARTING FROM $4E35 THE INTERPRETER ALLOCATES THE FOLLOWING RAM CELLS
...
USR             equ     NMIUSR+$03      ; (3) "USR (x)" jump  <-- in $40EF/$40F0 the user can store the address of a specific machine language routine
...
WORDS:  defb    'E'+$80,"ND"            ; from here the list contains the COMMANDS
        ...
        defb    'P'+$80,"OKE"
        defb    'D'+$80,"OKE"
        defb    'E'+$80,"LLIPSE"        ; added by Leonardo Miliani
...
WORDTB: defw    PEND
        ...
        defw    POKE
        defw    DOKE
        defw    ELLIPSE     ; added by Leonardo Miliani
...
ZPLUS   equ     $BC             ; +         <-- from here, there are the math operators
ZMINUS  equ     $BD             ; -
ZTIMES  equ     $BE             ; *
ZDIV    equ     $BF             ; /
ZMOD    equ     $C0             ; %
...
ZSGN    equ     $C9             ; SGN       <-- from here, there are the functions
ZPOINT  equ     $DF             ; ZPOINT    <-- if the user enters a custom function, between
                                ;               SGN and POINT, he/she must increment this pointer by 1
ZINSTR  equ     $E0             ; ZINSTR    <-- same here
ZLEFT   equ     $E8             ; LEFT$     <-- if the user enters a custom function anywhere,
...
SETVDP: call    CHKSCAR         ; no, check if sprite size follows
...
;ELLIPSE X,Y,A,B[,C]
; draw an ellipse, rotating it if needed
; X&Y: center of ellipse; A&B: main axes;
; C: color
;X1             TMPBFR1
;Y1             TMPBFR2
ELLA    equ     TMPBFR4
;X2             VIDEOBUFF
;Y2             VIDEOBUFF+$02
ELLA2   equ     VIDEOBUFF+$04
ELLB2   equ     VIDEOBUFF+$06
ELLFA   equ     VIDEOBUFF+$08
ELLSI   equ     VIDEOBUFF+$0A
ELLFB   equ     VIDEOBUFF+$0C
X3      equ     VIDEOBUFF+$0E
Y3      equ     VIDEOBUFF+$10

ELLIPSE:call    CHKG2M          ; check if in G2 mode
        call    CLRVDBF         ; clear VIDEOBUFF
        call    GETINT          ; get X coords.
        ld      (X3),A          ; store it into a temp buffer
        call    CHKSYN          ; Make sure ',' follows
        defb    ','
        call    GETINT          ; get Y coords,
        ld      (Y3),A          ; store it into a temp buffer
        call    CHKSYN          ; Make sure ',' follows
        defb    ','
        call    GETINT          ; get 'a' axis
        ld      (ELLA),A        ; store it into a temp buffer
        call    CHKSYN          ; Make sure ',' follows
        defb    ','
        call    GETINT          ; get 'b' axis
        ld      (ELLB2),A       ; store it into a temp buffer
        call    CLRPRM          ; check if color has been passed
; MAIN ALGORITHM
        push    HL              ; store HL
        push    DE              ; store DE
...
;20 X3=127:Y3=95
;30 A=60:B=30
;40 A2=A*A:B2=B*B:FA=4*A2
;50 X2=0:Y2=B
        ld      HL,(ELLB2)      ; load B
        ld      (Y2),HL         ; copy into Y2
        ld      DE,HL           ; copy into DE
        call    MUL16           ; B*B
        ld      (ELLB2),HL      ; store B2
        ld      HL,(ELLA)       ; load A
        ld      DE,HL           ; copy into DE
        call    MUL16           ; A*A
        ld      (ELLA2),HL      ; store A2
        add     HL,HL           ; A2*2
        add     HL,HL           ; A2*4
        ld      (ELLFA),HL      ; store FA
;60 SI=2*B2+A2*(1-2*B)
        ld      HL,(Y2)         ; load B
        add     HL,HL           ; 2*B
        ld      DE,$0001        ; DE=1
        ...
        sbc     HL,DE           ; HL=1-2*B
        ld      DE,(ELLA2)      ; load A2
        call    MUL16           ; HL=A2*(1-2*B2)
        ex      DE,HL           ; copy into DE
        ld      HL,(ELLB2)      ; load B2
        add     HL,HL           ; HL=2*B2
        call    MUL16           ; HL=2*B2+A2*(1-2*B)
        ld      (ELLSI),HL      ; store into SI
;70 PLOT X3+X2,Y3+Y2:PLOT X3-X2,Y3+Y2
;80 PLOT X3+X2,Y3-Y2:PLOT X3-X2,Y3-Y2
ELL0:   call    ELLPLT          ; plot pixels
;90 IF SI>=0 THEN SI=SI+FA2*(1-Y2):Y2=Y2-1
ELL4:   ld      HL,(ELLSI)      ; load SI       
        ld      BC,HL           ; copy into BC
        ld      DE,$0000        ; DE=0
        call    CMP16           ; SI>=0?
        jp      M,ELL5          ; no, SI<0, so jump over
        ld      HL,$0001        ; HL=1
        ld      DE,(Y2)         ; DE=Y2
        and     A               ; clear Carry
        sbc     HL,DE           ; HL=1-Y2
        ld      DE,(ELLFA)      ; DE=FA
        call    MUL16           ; HL=FA*(1-Y2)
        add     HL,BC           ; SI=SI+FA*(1-Y2)
        ld      (ELLSI),HL      ; store SI
        ld      HL,Y2           ; addess of Y2
        dec     (HL)            ; Y2=Y2-1
;100 SI=SI+B2*(4*X2+6)
ELL5:   ld      HL,(X2)         ; load X2
        add     HL,HL           ; X2*2
        add     HL,HL           ; X2*4
        ld      BC,$0006        ; BC=6
        add     HL,BC           ; 4*X2+6
        ld      DE,(ELLB2)      ; load B2
        call    MUL16           ; HL=B2*(4*X2+6)
        ld      DE,(ELLSI)      ; load SI
        add     HL,DE           ; SI=SI+B2*(4*X2+6)
        ld      (ELLSI),HL      ; store SI
;110 X2=X2+1
        ld      HL,(X2)         ; load X2
        inc     HL              ; X2=X2+1
        ld      (X2),HL         ; store SI
;120 IF B2*X2<=A2*Y2 THEN 70
        ld      DE,(ELLB2)      ; load B2
        call    MUL16           ; B2*X2
        ld      BC,HL           ; store B2*X2
        ld      DE,(ELLA2)      ; load A2
        ld      HL,(Y2)         ; load Y2
        call    MUL16           ; A2*Y2
        ld      DE,BC           ; load B2*X2
        ex      DE,HL           ; exchange
        call    CMP16           ; is B2*X2<=A2*Y2?
        jp      M,ELL0          ; yes, it's <, so repeat
        jp      Z,ELL0          ; yes, it's =, so repeat
;130 X2=A:Y2=0:FB2=4*B2
        ld      HL,(ELLA)       ; load A
        ld      (X2),HL         ; X2=A
        ld      HL,$0000        ; load 0
        ld      (Y2),HL         ; Y2=0
...
;140 SI=2*A2+B2*(1-2*A)
        ...
        sbc     HL,DE           ; HL=1-2*A
        ld      DE,(ELLB2)      ; DE=B2
        call    MUL16           ; HL=B2*(1-2*A)
        ld      DE,HL           ; store it
        ld      HL,(ELLA2)      ; HL=A2
        add     HL,HL           ; HL=2*A2
        add     HL,DE           ; SI=2*A2+B2*(1-2*A)
        ld      (ELLSI),HL      ; store SI
;150 PLOT X3+X2,Y3+Y2:PLOT X3-X2,Y3+Y2
;160 PLOT X3+X2,Y3-Y2:PLOT X3-X2,Y3-Y2
ELL6:   call    ELLPLT          ; plot pixels
;170 IF SI>=0 THEN SI=SI+FB2*(1-X2):X2=X2-1
        ld      DE,$0000        ; DE = 0
        ld      HL,(ELLSI)      ; HL=SI
        call    CMP16           ; is SI>=0?
        jp      M,ELL7          ; no, SI<0, so jump over
        ld      HL,$0001        ; HL=1
        ld      DE,(X2)         ; DE=X2
        and     A               ; clear Carry
        sbc     HL,DE           ; HL=1-X2
        ld      DE,(ELLFB)      ; DE=FB
        call    MUL16           ; HL=FB*(1-X2)
        ld      DE,(ELLSI)      ; DE=SI
        add     HL,DE           ; SI=SI+FB*(1-X2)
        ld      (ELLSI),HL      ; store SI
        ld      HL,X2           ; address of X2
        dec     (HL)            ; X2=X2-1
;180 SI=SI+A2*(4*Y2+6)
ELL7:   ld      HL,(Y2)         ; HL=Y2
        add     HL,HL           ; Y2*2
        ...
        add     HL,DE           ; HL=4*Y2+6
        ld      DE,(ELLA2)      ; DE=A2
        call    MUL16           ; HL=A2*(4*Y2+6)
        ld      DE,(ELLSI)      ; load SI
        add     HL,DE           ; SI=SI+A2*(4*Y2+6)
        ld      (ELLSI),HL      ; store SI
;190 Y2=Y2+1
        ld      HL,(Y2)         ; load Y2
        inc     HL              ; Y2=Y2+1
        ld      (Y2),HL         ; store Y2
;200 IF A2*Y2<=B2*X2 THEN 150
        ld      DE,(ELLA2)      ; DE=A2
        call    MUL16           ; HL=A2*Y2
        ...
        ld      DE,BC           ; DE=A2*Y2
        ex      DE,HL           ; exchange DE,HL
        call    CMP16           ; is A2*Y2<=B2*X2?
        jp      Z,ELL6          ; yes, =, so jump over
        jp      M,ELL6          ; yes, <, so jump over
ELLEND: pop     BC              ; no, finish - retrieve BC
        pop     DE              ; retrieve DE
        pop     HL              ; retrieve HL
        ret                     ; return to caller

;70/150 PLOT X3+X2,Y3+Y2:PLOT X3-X2,Y3+Y2
ELLPLT: ld      HL,(X3)         ; load X3
        ld      DE,(X2)         ; load X2
        add     HL,DE           ; X3+X2
        call    VALIDX          ; check X
        jp      C,ELL1          ; not valid, jump over
        ld      (X1),HL         ; store X
        ld      HL,(Y3)         ; load Y3
        ld      DE,(Y2)         ; load Y2
        add     HL,DE           ; Y3+Y2
...
ELL1:   ld      HL,(X3)         ; load X3
        ld      DE,(X2)         ; load X2
        and     A               ; clear Carry
        sbc     HL,DE           ; X3-X2
        call    VALIDX          ; check X
        jp      C,ELL2          ; not valid, jump over
        ld      (X1),HL         ; store X
        ld      HL,(Y3)         ; load Y3
        ld      DE,(Y2)         ; load Y2
        add     HL,DE           ; Y3+Y2
        ...
        ld      (Y1),HL         ; store Y
        call    CNTPLOT         ; plot pixel
;80/160 PLOT X3+X2,Y3-Y2:PLOT X3-X2,Y3-Y2
ELL2:   ld      HL,(X3)         ; load X3
        ld      DE,(X2)         ; load X2
        add     HL,DE           ; X3+X2
        call    VALIDX          ; check X
        jp      C,ELL3          ; not valid, jump over
        ld      (X1),HL         ; store X
        ld      HL,(Y3)         ; load Y3
        ld      DE,(Y2)         ; load Y2
        and     A               ; clear Carry
        ...
        ld      (Y1),HL         ; store Y
        call    CNTPLOT         ; plot pixel
ELL3:   ld      HL,(X3)         ; load X3
        ld      DE,(X2)         ; load X2
        and     A               ; clear Carry
        sbc     HL,DE           ; X3-X2
        call    VALIDX          ; check X
        jp      C,ELL4          ; not valid, jump over
        ld      (X1),HL         ; store X
        ld      HL,(Y3)         ; load Y3
        ld      DE,(Y2)         ; load Y2
        and     A               ; clear Carry
...
CLRPRM1:dec     HL              ; dec 'cos GETCHR INCs
        call    GETCHR          ; Get next character
...
PRTK2:  ld      DE,CHKEY1       ; message "KEY "
