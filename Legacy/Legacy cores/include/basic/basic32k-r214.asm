...
STRSPC          equ     WRKSPC+57H      ; Bottom of string space
LINEAT          equ     WRKSPC+59H      ; Current line number
BASTXT          equ     WRKSPC+5BH      ; Pointer to start of program   <-- actually this is the last value pre-filled by the firmware at startup
BUFFER          equ     WRKSPC+5EH      ; Input buffer
STACK           equ     WRKSPC+63H      ; Initial stack
CURPOS          equ     WRKSPC+0A8H     ; Character position on line
LCRFLG          equ     WRKSPC+0A9H     ; Locate/Create flag
TYPE            equ     WRKSPC+0AAH     ; Data type flag
DATFLG          equ     WRKSPC+0ABH     ; Literal statement flag
LSTRAM          equ     WRKSPC+0ACH     ; Last available RAM
TMSTPT          equ     WRKSPC+0AEH     ; Temporary string pointer
TMSTPL          equ     WRKSPC+0B0H     ; Temporary string pool
TMPSTR          equ     WRKSPC+0BCH     ; Temporary string
STRBOT          equ     WRKSPC+0C0H     ; Bottom of string space
CUROPR          equ     WRKSPC+0C2H     ; Current operator in EVAL
LOOPST          equ     WRKSPC+0C4H     ; First statement of loop
DATLIN          equ     WRKSPC+0C6H     ; Line of current DATA item
FORFLG          equ     WRKSPC+0C8H     ; "FOR" loop flag
LSTBIN          equ     WRKSPC+0C9H     ; Last byte entered
READFG          equ     WRKSPC+0CAH     ; Read/Input flag
BRKLIN          equ     WRKSPC+0CBH     ; Line of break
NXTOPR          equ     WRKSPC+0CDH     ; Next operator in EVAL
ERRLIN          equ     WRKSPC+0CFH     ; Line of error
CONTAD          equ     WRKSPC+0D1H     ; Where to CONTinue
PROGND          equ     WRKSPC+0D3H     ; End of program
VAREND          equ     WRKSPC+0D5H     ; End of variables
ARREND          equ     WRKSPC+0D7H     ; End of arrays
NXTDAT          equ     WRKSPC+0D9H     ; Next data item
FNRGNM          equ     WRKSPC+0DBH     ; Name of FN argument
FNARG           equ     WRKSPC+0DDH     ; FN argument value
FPREG           equ     WRKSPC+0E1H     ; Floating point register
FPEXP           equ     FPREG+03H       ; Floating point exponent
SGNRES          equ     WRKSPC+0E5H     ; Sign of result
...
PBUFF           equ     CONTROLKEYS+01H ; Number print buffer
...
ZPLUS   equ     0B7H            ; +         <-- from here, there are the math operators
ZMINUS  equ     0B8H            ; -
ZTIMES  equ     0B9H            ; *
ZDIV    equ     0BAH            ; /
ZOR     equ     0BDH            ; OR
...
ZSGN    equ     0C1H            ; SGN       <-- from here, there are the functions
ZPOINT  equ     0D7H            ; POINT     ; if the user enters a custom function,
ZLEFT   equ     0DFH            ; LEFT$     ; he/she must increment these two pointers by 1
...
; change the colors of the screen - Sintax is COLOR a,b,c where:
