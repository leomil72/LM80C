...
WLCBPDAT:       defb    $07,%10111011,$04,$56,$05,$00,$0A,$0F
NOBPDAT:        defb    $04,$00,$05,$00,$0A,$00,$07,%10111111
...
LOADMAP:        dec     B               ; decrement row # (rows go from 0 to 7)
                ...
                ld      D,B             ; reset D
                add     HL,DE           ; find the current column - now (HL) stores the pressed key
                ld      A,(LASTKEYPRSD) ; load the last key pressed
                cp      (HL)            ; is it the same key?
                jr      NZ,NEWKEY       ; no, it's a new key
                ld      BC,HL           ; store address of current keycode
                ld      HL,(TMRCNT)     ; load current system timer
                ld      DE,(KEYTMR)     ; load auto-repeat timer
                ld      A,(STATUSKEY)   ; yes, load current status of auto-repeat
                cp      $01             ; is it initial pressure?
                jr      NZ,CHKAUTO      ; no, jump over
                xor     A               ; clear Carry
                sbc     HL,DE           ; startint time - actual time
                ld      D,$00           ; 64 ths/s > 640 ms
                ld      A,(KEYDEL)
                ld      E,A
                call    CMP16           ; time elapsed is >= $40?
                jp      C,LVKBRDCHK2    ; no, so leave
SETNEWAUTO:     ld      A,$02           ; yes set status to...
                ld      (STATUSKEY),A   ; ...auto-repeat
                ld      HL,(TMRCNT)     ; load system timer
                ld      (KEYTMR),HL     ; store starting time
                ld      HL,BC           ; retrieve address of current keycode
                jp      SENDKEY         ; send key
CHKAUTO:        xor     A               ; clear Carry
                sbc     HL,DE           ; computer interval between initial pressure of key and current time
                ld      D,$00
                ld      A,(AUTOKE)
                ld      E,A             ; auto-repeat delay into DE
                call    CMP16           ; check if interval is greater than delay
                jp      C,LVKBRDCHK2    ; no, so leave                
                jp      SETNEWAUTO      ; set new loop and send key
NEWKEY:         xor     A               ; set initial state...
                inc     A               ; ...for auto-repeat...
                ld      (STATUSKEY),A   ; ...1=pressure init
                ld      BC,(TMRCNT)     ; load current system timer
                ld      (KEYTMR),BC     ; set starting time
SENDKEY:        ld      A,(HL)          ; then, load key...
...
KBMAP_SFT:      defb '!',12,14,3,' ',16,'Q',34          ; 12=CLEAR  14=CTRL  3=RUN/STOP 16=C=   34="
                ...
                defb 26,13,211,'@',5,6,22,23            ; 26=INSERT, 211=€  5=F4  6=F5  22=F6  23=F7
