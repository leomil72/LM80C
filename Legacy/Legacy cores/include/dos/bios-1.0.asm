...
; check for errors
CF_CHKERR:  in      A,(CF_STAT)     ; read status register
            rra                     ; move error bit #0 into Carry
            ret                     ; return to caller


; raw read 1 byte from CF
CF_RAWRDBT: call    CF_DAT_RDY      ; check if data is ready
            in      A,(CF_DATA)     ; read byte
            ret                     ; return to caller
...
CF_RD_CMD:  push    BC              ; store BC
            ...
            ld      B,$00           ; read 256 words (512 bytes per sector)
...
CF_WR_SEC:  push    BC              ; store BC
            ...
            ld 	    HL,(IOBUFF)     ; get starting address of I/O buffer
            ld      E,$02           ; repeat 2 times
CF_WR_SEC1: ld      B,$00           ; read 256 bytes per cycle
CF_WR_SEC2: ld      A,(HL)          ; load byte
            out     (CF_DATA),A     ; send byte to CF
            inc     HL              ; next location
            djnz    CF_WR_SEC2      ; repeat
            dec     E               ; 2 cycles
            jr      NZ,CF_WR_SEC1   ; repeat
