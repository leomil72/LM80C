...
RX_CHA_AVAILABLE:
                ...
                ld      a,(INKEYFLG)    ; check if INKEY is sniffing the serial
                cp      $01             ; 1=sniff / 0=don't
                ld      a,h             ; recover char
                jr      nz,CONT_RX      ; no, continue
                ld      (TMPKEYBFR),a   ; store it into the temp key buffer
                jr      RX_CH_AV_EXIT   ; and exit without sending the char to the serial buffer
CONT_RX:        push    af              ; store it
...
RX_CH_AV_EXIT:  pop     hl              ; recover HL
                pop     af              ; and A
                ei                      ; re-enable interrupts
                reti                    ; and exit
...
CH3_TIMER:      push    af              ; save regs. A,
                ...
                ld      (TMPKEYBFR),a   ; delete temp key buffer
...
EXCH3T:         call    MNGSNDS         ; call the tone managemenet
                pop     hl              ; recover HL
                pop     de              ; recover DE
                pop     bc              ; BC,
                pop     af              ; and A
