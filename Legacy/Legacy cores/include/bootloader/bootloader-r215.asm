...
NOTWRAP:        ld      (serInPtr),HL   ; store the new pointer
                ...
                jp      A_RTS_OFF       ; else stop receiving further chars
...
A_RTS_OFF:      push    AF              ; store A
                ...
                or      00101000b       ; TX enabe; RTS disable
...
A_RTS_ON:       push    AF              ; store A
                ...
                out     (SIO_CA),A
                ld      A,(SERABITS)    ; load data bits
                or      00101010b       ; TX enable; RTS enable
                out     (SIO_CA),A      ; send setting
                pop     AF              ; retrieve A
                ret                     ; return
...
TXA:            di                      ; disable INTs
                ...
                jr      NZ,CHK_TXA      ; no, jump over
                call    CHAR2VID        ; yes, send char to video buffer
CHK_TXA:        ld      A,(SERIALS_EN)  ; load serials status
...
WASTETIME       nop                     ; ...some time to...
                ...
                ret
;-------------------------------------------------------------------------------
ECHOCAR:        call    TXA             ; echoes back the pressed key,
                ld      A,CR            ; then CR
                call    TXA             ; rst     $08
                ret
;-------------------------------------------------------------------------------
...
SIO_B_SETS:     defb    00000001b       ; write into WR0: select WR1
                defb    00000100b       ; write into WR1: status affects interrupt vectors
                defb    00000010b       ; write into WR0: select WR2
                defb    00000000b       ; write into WR2: set interrupt vector, but bits D3/D2/D1 of this vector
