...
FWVER:          defm    'FW R3.0',$20,__date__,$20,__time__,$00
;------------------------------------------------------------------------------
...
RX_CHA_AVAILABLE:
                ...
                cp      $20             ; is it another control char?
...
; this sub is called both from the ISR "RX_CHA_AVAILABLE" and when
...
NOTWRAP:        ld      (serInPtr),HL   ; store the new pointer
                ...
                ld      A,(SERIALS_EN)  ; check if serial 0 is open
                dec     A               ; if bit 0 is 1 then serial 0 is open and...
                call    Z,A_RTS_OFF     ; ...stop receiving further chars
                ret
...
A_RTS_OFF:      push    AF              ; store A
                ...
                or      %00101000       ; TX enabe; RTS disable
...
SPEC_RX_CONDITON:
                ...
                ld      C,PIO_DB        ; PIO port B
                in      B,(C)           ; read status LEDs
                set     4,C             ; set 5th pin ON
                out     (C),B           ; send new setting
                pop     BC              ; retrieve BC
                ei                      ; re-enable interrupts
...
RXA:            ld      A,(serBufUsed)  ; load the buffer size
                ...
                jr      Z,RXA           ; if it's empty, wait for a char
...
TXA:            push    AF              ; store original flags
                ld      (TMPBFR3),A     ; store char
...
CNTTXA:         push    AF              ; store current P/V flag
                di                      ; disable INTs
CHK_TXA:        ld      A,(SERIALS_EN)  ; load serial status
                dec     A               ; check if serial 1 is enabled
                jp      NZ,TXA_EXIT     ; no, jump over
                ld      A,(TMPBFR3)     ; retrieve char
...
EXTXA:          pop     AF              ; retrieve original flags
                ret                     ; return
...
INIT_HW:        ld      HL,TEMPSTACK    ; load temp stack pointer
                ...
                ld      E,$01           ; E chooses the video mode:
                                        ; 0: text; 1:graphics 1; 2:graphics 2; 3:multicolor; 4:extended G2
                call    initVDP         ; set video display
                xor     A
                ld      I,A             ; set high byte of interrupt vectors to point to page 0
                im      2               ; interrupt mode 2
                ei                      ; enable interrupts
PRINTWELCOME:   xor     A               ; A=0 so...
                ld      (KBDNPT),A      ; ...inputs don't come from keyboard
                inc     A               ; A=1...
                ld      (PRNTVIDEO),A   ; ...to activate the video buffer print
...
CORW:           ld      A,(CHR4VID)     ; look for a pressed key
                and     %11011111       ; only UPPERCASE char
                ld      (CHR4VID),A     ; store UPPERCASE char
                cp      'C'             ; cold start?
...
HELLOWRLD:      ld      C,$09           ; 8 LEDs to be turned off + 1 more step to turn off the last LED
                ld      A,11001111b     ; set PIO port B to mode 3 (mode control)
                out     (PIO_CB),A
                xor     A               ; set pins of PIO port B to OUTPUT
                out     (PIO_CB),A
                inc     A               ; LSB on
