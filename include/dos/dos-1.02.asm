...
LSTFILES5:  inc     BC              ; increment sector counter
            ...
            call    HL2DE           ; load data area sector into DE
...
DSKUNDFL:   call    CLRIOBF         ; clear I/O
            call    CHKDSKVAL       ; check DOS version & load disk details
            jp      C,DOSVERSERR    ; if Carry is set, raise DOS version error
            call    FNDFREDIR       ; find a free entry
DSKUNDFL1:  ret     C               ; return if entries are finished
            ...
            call    CHATOZ          ; transfort it into a letter
...
DSKUNDPR:   ld      A,(HL)          ; retrieve char from filename
            ...
            pop     BC              ; retrieve BC
            pop     HL              ; retrieve HL
DSKUNDFL2:  call    FNDFREDIR4      ; goto next entry
            jr      DSKUNDFL1       ; repeat
...
; *****************************************************************************
FNDFREDIR:  call    LDMSCT          ; read Master Sector
...
FNDFREDIR2: ld      A,(IX)          ; load 1st char of entry name
            ...
            jr      Z,FNDFREDIR3    ; yes, found an entry
FNDFREDIR4: push    BC              ; no, store BC
...
LDMSCT:     ld      BC,$0000        ; LSB of sector
LDMSCT1:    ld      D,B             ; MSB of sector
            ld      E,B
            call    CF_SETSTR       ; set sector
            call    CF_RD_SEC       ; read sector
            ret
...
RND8:       push    BC
            ld      A,(TMRCNT)      ; load LSB of sys-timer
            ld      B,A             ; copy into B
...
PRN32ASCII: call    HL2DE           ; load MSB into DE  <-- entry for 32-bit
