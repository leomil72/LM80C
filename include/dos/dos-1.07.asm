...
; R1.07 - 20240112 - revised ERASE command
;
;------------------------------------------------------------------------------
...
OPFLRS1:    call    CF_SETSTR       ; point to 1st sector 
...
CLOSFIL1:   call    CF_STANDBY      ; put disk into standby
...
; *****************************************************************************
EXIST:      call    CHKSQFL         ; is there a seq. file opened?
            ld      E,D0            ; prepare a file open error
            jp      NZ,ERROR        ; yes, exit with error
            call    CHKDSKVAL       ; check DOS version & load disk details
            ld      E,D7            ; prepare a DOS version error
            jp      C,ERROR         ; if Carry is set, raise DOS version error
            push    HL              ; store code string pointer
            call    GETSTR          ; get string (error if it isn't)
            call    CHKFN2          ; store string and get pointers
            call    CLRIOBF         ; clear I/O
            call    CHKFLEXT        ; check if file already exists
            pop     HL              ; retrieve code string pointer
            ld      A,$00           ; reset A
            rl      A               ; store Carry into A: 0=file not exist, 1=file exist
            ld      E,A
            call    CF_STANDBY
            ld      A,E
            jp      PASSA           ; return A
...
; *****************************************************************************
GET:        call    CHKSQFL         ; is there a seq. file opened?
            ld      A,D0            ; prepare a file open error
            jr      Z,GETER         ; no, exit with error
            ld      A,(SEQFLS)      ; yes, check mode
            or      A               ; is it write?
            jp      NZ,FCERR        ; yes, raise error
            call    DEINT           ; no, get file number
            ld      A,E             ; check if...
            or      A               ; ...it's zero
            jp      Z,GETZER        ; yes, reset file pointer
            ld      A,(SEQFL)       ; no, compare file opened
            cp      E               ; with file passed
            jp      NZ,FCERR        ; error if different
...
GETZER:     push    HL
            ld      DE,(MSW_SCT)    ; reload MSW of file
            ld      (SEQSCTM),DE    ; restore
            ld      BC,(LSW_SCT)    ; reload LSW of file
            ld      (SEQSCTL),BC    ; restore
            call    OPFLRS1         ; reload 1st sector and reset pointer
            pop     HL              ; retrieve code string pointer
            xor     A               ; set A to 0 for "no errors"
            jp      PASSA           ; return A
...
FIL_ERASE:  call    CHKSQFL         ; check if a seq. file is open
            ...
            ld      A,(IX)          ; load first char of file name
            add     A,$80           ; set 8th bit to...
            ld      (IX),A          ; ...mark file as deleted (quick erase)
...
DSKUNDFL1:  jr      C,DSKUNDFL3     ; return if entries are finished
            ...
            cp      $80             ; is it a deleted entry (>=$80)?
            jr      C,DSKUNDFL2     ; no, jump over
            sub     $80             ; retrieve old char
