...
SCR_SIZE_H      equ     WRKSPC+0EEH     ; (1) screen height (it can be 24/48/192: 24 for text, 48 for MC, 192 for graphics)
...
TMPBFR1         equ     WRKSPC+0FCH     ; (2) word for general purposes use (temp. buffer for 1 or 2 bytes)
TMPBFR2         equ     WRKSPC+0FEH     ; (2) word for general purposes use (temp. buffer for 1 or 2 bytes)
TMPBFR3         equ     WRKSPC+100H     ; (2) word for general purposes use (temp. buffer for 1 or 2 bytes)
VIDEOBUFF       equ     WRKSPC+102H     ; (40) buffer used for video scrolling and other purposes
...
; - - - - - - - - - - - - - - - - - - -   ...TO HERE. DO NOT ADD ANYTHING RELATED TO VPD OUT OF THIS RANGE,
                                        ; OTHERWISE YOU WILL HAVE TO CHANGE THE POINTER IN "EMPTY_RAM" FUNCTION
...
ZPLUS   equ     0B3H            ; +         <-- from here, there are the math operators
ZMINUS  equ     0B4H            ; -
ZTIMES  equ     0B5H            ; *
ZDIV    equ     0B6H            ; /
ZOR     equ     0B9H            ; OR
...
ZSGN    equ     0BDH            ; SGN       <-- from here, there are the functions
ZPOINT  equ     0D3H            ; POINT     ; if the user enters a custom function,
ZLEFT   equ     0DBH            ; LEFT$     ; he/she must increment these two pointers by 1
...
PROCES: ld      A,C             ; Get character
        ...
        cp      CRSRGT          ; Is it Cursor Left?
        jp      Z,MV_CRS        ; Yes - Nove cursor to left
        cp      CRSUP           ; Is it Cursor Left?
        jp      Z,MV_CRS        ; Yes - Nove cursor to left
        cp      CRSDN           ; Is it Cursor Left?
        jp      Z,MV_CRS        ; Yes - Nove cursor to left
...
; 3=multicolor mode (64x48 pixels)
...
SETVDP: push    af              ; store A
        ...
        or      L
        pop     HL              ; Restore code string address
        call    Z,CURSOR_ON     ; enable cursor if not in program
        ei                      ; re-enable interrupts
        pop     hl              ; restore HL
        pop     de              ; restore DE
        pop     af              ; restore A
