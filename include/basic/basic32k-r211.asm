...
TMRCNT          equ     WRKSPC+0E9H     ; (4) TMR counter for 1/100 seconds
...
SCR_SIZE_B      equ     SCR_MODE+01H    ; (2) screen size in bytes
SCR_NAM_TB      equ     SCR_SIZE_B+02H  ; (2) video name table address
...
TMPBFR1         equ     FRGNDCLR+01H    ; (2) word for general purposes use (temp. buffer for 1 or 2 bytes)
...
SETVDP: push    AF              ; store A
        ...
        xor     A               ; reset A
        ld      D,A             ; reset D: D is 0 for no power-on msg., and 1 for power-on msg.
        push    DE              ; store DE
        push    HL              ; store HL
        di                      ; disable interrupts
        call    initVDP         ; initialize VDP
        push    HL              ; Save code string address
        ld      HL,(LINEAT)     ; Get current line number
        inc     HL              ; -1 means direct statement
        ld      A,H
...
CHKCLR: cp      $00             ; is it 0?
        jp      Z,SNERR         ; yes, raise a SN error
        cp      $10             ; is it in range 1~15?
        jp      NC,SNERR        ; yes, raise a SN error
        ret                     ; param is OK, can return


; PLOT X,Y[,color]
; plota pixel in graphic mode 2
PLOT:   ld      A,(SCR_MODE)    ; check screen mode
        cp      $02             ; actually, we can paint only in G2
        jp      NZ,FCERR        ; no G2, print an Illegal Function call
...
CNTPLOT:push    HL              ; store HL
        ...
        pop     HL              ; retrieve HL
        ret                     ; return to caller
PXLSET: defb    80h,40h,20h,10h,08h,04h,02h,01h ; pixel to be put ON
...
DX      equ     VIDEOBUFF+$0C
DY      equ     VIDEOBUFF+$0E
DRAW:   ld      A,(SCR_MODE)    ; check screen mode
        cp      $02             ; actually, we can paint only in G2
        jp      NZ,FCERR        ; no G2, print an Illegal Function call
...
YI      equ     VIDEOBUFF+$08
DC      equ     VIDEOBUFF+$0A
CIRCLE: ld      A,(SCR_MODE)    ; check screen mode
        cp      $02             ; actually, we can paint only in G2
        jp      NZ,FCERR        ; no G2, print an Illegal Function call
        call    CLRVDBF         ; clear VIDEOBUFF
        call    GETINT          ; get X coords.
        ...
        defb    ','
        call    GETINT          ; get Y coords,
        ld      (YC),A          ; store into a temp buffer
        call    CHKSYN          ; Make sure ',' follows
        defb    ','
...
DRWCRL: ld      HL,(XC)
        ...
        jp      C,CNTCL1        ; if Carry is set, Y is not valid
        call    CNTPLOT         ; plot pixel
CNTCL1: xor     A               ; clear Carry
        ...
        ld      (Y1),HL         ; store Y
        call    VALIDY          ; check if Y is valid (0~191)
        jp      C,CNTCL2        ; if Carry is set, Y is not valid
        call    CNTPLOT         ; plot pixel
CNTCL2: ld      HL,(XC)
        ld      DE,(XI)
        add     HL,DE           ; X=XC+XI
        ...
        ld      (Y1),HL         ; store Y
        call    VALIDY          ; check if Y is valid (0~191)
        jp      C,CNTCL3        ; if Carry is set, Y is not valid
        call    CNTPLOT         ; plot pixel
CNTCL3: xor     A               ; clear Carry
        ...
        ld      (Y1),HL         ; store Y
        call    VALIDY          ; check if Y is valid (0~191)
        jp      C,CNTCL4        ; if Carry is set, Y is not valid
        call    CNTPLOT         ; plot pixel
CNTCL4: ld      HL,(XC)
        ld      DE,(YI)
        add     HL,DE           ; X=XC+YI
        ...
        ld      (Y1),HL         ; store Y
        call    VALIDY          ; check if Y is valid (0~191)
        jp      C,CNTCL5        ; if Carry is set, Y is not valid
        call    CNTPLOT         ; plot pixel
CNTCL5: xor     A               ; clear Carry
        ...
        ld      (Y1),HL         ; store Y
        call    VALIDY          ; check if Y is valid (0~191)
        jp      C,CNTCL6        ; if Carry is set, Y is not valid
        call    CNTPLOT         ; plot pixel
CNTCL6: ld      HL,(XC)
        ld      DE,(YI)
        add     HL,DE           ; X=XC+YI
        ...
        ld      (Y1),HL         ; store Y
        call    VALIDY          ; check if Y is valid (0~191)
        jp      C,CNTCL7        ; if Carry is set, Y is not valid
        call    CNTPLOT         ; plot pixel
CNTCL7: xor     A               ; clear Carry
        ...
        ret                     ; return to caller
VALIDX: ld      A,H
        or      A               ; check if H is 0 (this means that X is in range 0~255 and not negative)
        ret     Z               ; yes, we can return (C is clear)
        scf                     ; set Carry flag to raise error
        ret
VALIDY: ld      A,H
        or      A               ; check if H is 0 (this means that Y is in range 0~255 and not negative)
...
CLRPRM: ld      A,(FRGNDCLR)    ; load foreground color
        ...
        ld      (TMPBFR3),A     ; store color into a temp buffer
        ret                     ; return to caller
