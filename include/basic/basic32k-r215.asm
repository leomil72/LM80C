...
HOME            equ     19H             ; Home (cursor at 0,0)
ESC             equ     1BH             ; Escape
SPC             equ     20H             ; Space
DEL             equ     7FH             ; Delete
; cursor ASCII codes
...
WRKSPC          equ     8045H           ; (3) BASIC Work space
USR             equ     WRKSPC+3H       ; (3) "USR (x)" jump  <-- in $8049/804A the user can store the address of a specific machine language routine
OUTSUB          equ     WRKSPC+6H       ; (1) "out p,n"
OTPORT          equ     WRKSPC+7H       ; (2) Port (p)
DIVSUP          equ     WRKSPC+9H       ; (1) Division support routine
DIV1            equ     WRKSPC+0AH      ; (4) <- Values
DIV2            equ     WRKSPC+0EH      ; (4) <-   to
DIV3            equ     WRKSPC+12H      ; (3) <-   be
DIV4            equ     WRKSPC+15H      ; (2) <-inserted
SEED            equ     WRKSPC+17H      ; (35) Random number seed  <-- starting address of a seed table
LSTRND          equ     WRKSPC+3AH      ; (4) Last random number
INPSUB          equ     WRKSPC+3EH      ; (1) #INP (x)" Routine
INPORT          equ     WRKSPC+3FH      ; (2) PORT (x)
NULLS           equ     WRKSPC+41H      ; (1) Number of nulls
LWIDTH          equ     WRKSPC+42H      ; (1) Terminal width
COMMAN          equ     WRKSPC+43H      ; (1) Width for commas
NULFLG          equ     WRKSPC+44H      ; (1) Null after input byte flag
CTLOFG          equ     WRKSPC+45H      ; (1) Control "O" flag
LINESC          equ     WRKSPC+46H      ; (2) Lines counter
LINESN          equ     WRKSPC+48H      ; (2) Lines number
CHKSUM          equ     WRKSPC+4AH      ; (2) Array load/save check sum
NMIFLG          equ     WRKSPC+4CH      ; (1) Flag for NMI break routine
BRKFLG          equ     WRKSPC+4DH      ; (1) Break flag
RINPUT          equ     WRKSPC+4EH      ; (3) Input reflection
POINT           equ     WRKSPC+51H      ; (3) "POINT" reflection (unused)
PSET            equ     WRKSPC+54H      ; (3) "SET"   reflection
STRSPC          equ     WRKSPC+57H      ; (2) Bottom of string space
LINEAT          equ     WRKSPC+59H      ; (2) Current line number
BASTXT          equ     WRKSPC+5BH      ; (3) Pointer to start of program   <-- actually this is the last value pre-filled by the firmware at startup
BUFFER          equ     WRKSPC+5EH      ; (5) Input buffer
STACK           equ     WRKSPC+63H      ; (69) Initial stack
CURPOS          equ     WRKSPC+0A8H     ; (1) Character position on line
LCRFLG          equ     WRKSPC+0A9H     ; (1) Locate/Create flag
TYPE            equ     WRKSPC+0AAH     ; (1) Data type flag
DATFLG          equ     WRKSPC+0ABH     ; (1) Literal statement flag
LSTRAM          equ     WRKSPC+0ACH     ; (2) Last available RAM
TMSTPT          equ     WRKSPC+0AEH     ; (2) Temporary string pointer
TMSTPL          equ     WRKSPC+0B0H     ; (12) Temporary string pool
TMPSTR          equ     WRKSPC+0BCH     ; (4) Temporary string
STRBOT          equ     WRKSPC+0C0H     ; (2) Bottom of string space
CUROPR          equ     WRKSPC+0C2H     ; (2) Current operator in EVAL
LOOPST          equ     WRKSPC+0C4H     ; (2) First statement of loop
DATLIN          equ     WRKSPC+0C6H     ; (2) Line of current DATA item
FORFLG          equ     WRKSPC+0C8H     ; (1) "FOR" loop flag
LSTBIN          equ     WRKSPC+0C9H     ; (1) Last byte entered
READFG          equ     WRKSPC+0CAH     ; (1) Read/Input flag
BRKLIN          equ     WRKSPC+0CBH     ; (2) Line of break
NXTOPR          equ     WRKSPC+0CDH     ; (2) Next operator in EVAL
ERRLIN          equ     WRKSPC+0CFH     ; (2) Line of error
CONTAD          equ     WRKSPC+0D1H     ; (2) Where to CONTinue
PROGND          equ     WRKSPC+0D3H     ; (2) End of program
VAREND          equ     WRKSPC+0D5H     ; (2) End of variables
ARREND          equ     WRKSPC+0D7H     ; (2) End of arrays
NXTDAT          equ     WRKSPC+0D9H     ; (2) Next data item
FNRGNM          equ     WRKSPC+0DBH     ; (2) Name of FN argument
FNARG           equ     WRKSPC+0DDH     ; (4) FN argument value
FPREG           equ     WRKSPC+0E1H     ; (3) Floating point register
FPEXP           equ     FPREG+03H       ; (1) Floating point exponent
SGNRES          equ     WRKSPC+0E5H     ; (1) Sign of result
TMRCNT          equ     WRKSPC+0E6H     ; (4) TMR counter for 1/100 seconds
; - - - - - - - - - - - - - - - - - - -   VIDEO REGISTERS - FROM HERE...
SCR_SIZE_W      equ     TMRCNT+04H      ; (1) screen width (it can be either 40 chars or 32 chars/bytes)
SCR_SIZE_H      equ     SCR_SIZE_W+01H  ; (1) screen height (it can be 24/48/192: 24 for text, 48 for MC, 192 for graphics)
SCR_MODE        equ     SCR_SIZE_H+01H  ; (1) screen mode (0=text, 1=G1, 2=G2, 3=MC, 4=ExG2)
SCR_NAM_TB      equ     SCR_MODE+02H    ; (2) video name table address
SCR_CURS_X      equ     SCR_NAM_TB+02H  ; (1) cursor X
SCR_CURS_Y      equ     SCR_CURS_X+01H  ; (1) cursor Y
SCR_CUR_NX      equ     SCR_CURS_Y+01H  ; (1) new cursor X position
SCR_CUR_NY      equ     SCR_CUR_NX+01H  ; (1) new cursor Y position
SCR_ORG_CHR     equ     SCR_CUR_NY+01H  ; (1) original char positioned under the cursor
CRSR_STATE      equ     SCR_ORG_CHR+01H ; (1) state of cursor (1=on, 0=off)
LSTCSRSTA       equ     CRSR_STATE+01H  ; (1) last cursor state
PRNTVIDEO       equ     LSTCSRSTA+01H   ; (1) print on video buffer (1=on / 0=off) set to off on graphic only modes
CHR4VID         equ     PRNTVIDEO+01H   ; (1) char for video buffer
FRGNDCLR        equ     CHR4VID+01H     ; (1) foreground color as set by SCREEN or COLOR commands
BKGNDCLR        equ     FRGNDCLR+01H    ; (1) background color as set by SCREEN or COLOR commands
TMPBFR1         equ     BKGNDCLR+01H    ; (2) word for general purposes use (temp. buffer for 1 or 2 bytes)
TMPBFR2         equ     TMPBFR1+02H     ; (2) word for general purposes use (temp. buffer for 1 or 2 bytes)
TMPBFR3         equ     TMPBFR2+02H     ; (2) word for general purposes use (temp. buffer for 1 or 2 bytes)
VIDEOBUFF       equ     TMPBFR3+02H     ; (40) buffer used for video scrolling and other purposes
VIDTMP1         equ     VIDEOBUFF+028H  ; (2) temporary video word
VIDTMP2         equ     VIDTMP1+002H    ; (2) temporary video word
...
CHASNDDTN       equ     VIDTMP2+002H    ; (2) sound Ch.A duration (1/100s)
CHBSNDDTN       equ     CHASNDDTN+002H  ; (2) sound Ch.B duration (1/100s)
CHCSNDDTN       equ     CHBSNDDTN+002H  ; (2) sound Ch.C duration (1/100s)
KBTMP           equ     CHCSNDDTN+002H  ; (1) temp cell used by keyboard scanner
TMPKEYBFR       equ     KBTMP+001H      ; (1) temp buffer for last key pressed
LASTKEYPRSD     equ     TMPKEYBFR+001H  ; (1) last key code pressed
CONTROLKEYS     equ     LASTKEYPRSD+01H ; (1) flags for control keys (bit#0=SHIFT; bit#1=CTRL; bit#2=C=)
...
SERIALS_EN      equ     CONTROLKEYS+01h ; serial ports status: bit 0 for Port1(A), bit 1 for Port2(B): 0=OFF, 1=ON
SERABITS        equ     SERIALS_EN+$01  ; serial port A data bits
PBUFF           equ     SERABITS+01H    ; Number print buffer
MULVAL          equ     PBUFF+0DH       ; Multiplier
PROGST          equ     MULVAL+03H      ; Start of program text area
STLOOK          equ     PROGST+064H     ; (WRKSPC+16DH) Start of memory test
...
GM      equ     2AH             ; No Graphics Mode
SC      equ     2CH             ; Serial Configuration
...
WORDS:  defb    'E'+80H,"ND"            ; from here the list contains the COMMANDS
        ...
        defb    'S'+80H,"OUND"          ; added by Leonardo Miliani
        defb    'V'+80H,"OLUME"         ; added by Leonardo Miliani
        defb    'P'+80H,"AUSE"          ; added by Leonardo Miliani
        defb    'C'+80H,"OLOR"          ; added by Leonardo Miliani
        defb    'P'+80H,"LOT"           ; added by Leonardo Miliani
        defb    'D'+80H,"RAW"           ; added by Leonardo Miliani
        defb    'C'+80H,"IRCLE"         ; added by Leonardo Miliani
        defb    'S'+80H,"ERIAL"         ; added by Leonardo Miliani
        defb    'L'+80H,"INES"
        defb    'C'+80H,"LS"            ; restored command
        defb    'W'+80H,"IDTH"
        defb    'S'+80H,"YS"            ; added by Leonardo Miliani
        defb    'S'+80H,"ET"
        ...
        defb    'I'+80H,"NKEY"          ; <-- added by Leonardo Miliani
        defb    'P'+80H,"OINT"
        defb    'L'+80H,"EN"
        defb    'S'+80H,"TR$"
        defb    'V'+80H,"AL"
...
ZPLUS   equ     0B8H            ; +         <-- from here, there are the math operators
ZMINUS  equ     0B9H            ; -
ZTIMES  equ     0BAH            ; *
ZDIV    equ     0BBH            ; /
ZOR     equ     0BEH            ; OR
...
ZSGN    equ     0C2H            ; SGN       <-- from here, there are the functions
ZPOINT  equ     0D8H            ; POINT     ; if the user enters a custom function,
ZLEFT   equ     0E0H            ; LEFT$     ; he/she must increment these two pointers by 1
...
PROCES: ld      A,C             ; Get character
        ...
        cp      HOME            ; IS it home?
        jp      Z,ATHOME        ; Positon cursor at 0,0
        cp      CS              ; Is it clear screen?
        jp      Z,CLRSCR        ; Yes - Clear screen & kill
        cp      CR              ; Is it enter?
        jp      Z,ENDINP        ; Yes - Terminate input
        cp      CTRLU           ; Is it control "U"?
        jp      Z,KILIN         ; Yes - Get another line
        cp      CRSLFT          ; Is it Cursor Left?
        jp      Z,MV_CRS        ; Yes - Nove cursor to left
        cp      CRSRGT          ; Is it Cursor right?
        jp      Z,MV_CRS        ; Yes - Nove cursor to right
        cp      CRSUP           ; Is it Cursor up?
        jp      Z,MV_CRS        ; Yes - Nove cursor to up
        cp      CRSDN           ; Is it Cursor down?
        jp      Z,MV_CRS        ; Yes - Nove cursor to down
...
; move cursor
MV_CRS: call    OUTC            ; send code to screen
        jp      TTYLIN          ; start input again

; clear screen and/or position cursor at home (0,0)
CLRSCR: call    OUTC            ; send char to screen (clear screen)
...
DOTAB:  push    AF              ; Save token
        ...
        ld      C,SPC           ; for SPC we use SPACE char
        sub     ZSPC            ; Was it "SPC(" ?
        push    HL              ; Save code string address
...
INPUT:  call    IDTEST          ; Test for illegal direct
        ...
        cp      22H             ; '"' ; Is there a prompt string?
...
QTSTR:  ld      B,22H           ; '"'           ; Terminating quote
        ld      D,B             ; Quote to D
...
; a,b,c range between 1 and 15 (0 is transparent and is not supported)
...
PXLSET: defb    $80,$40,$20,$10,$08,$04,$02,$01 ; pixel to be set ON
        ; where R(X/8)=> 0=80h, 1=40h, 2=20h, 3=10h, 4=08h, 5=04h, 6=02h, 7=01h
...
SERIAL: call    GETINT          ; get port #
        ...
        ld      A,(PRTNUM)      ; yes, so reset the channel. First, load port number
...
SETPAR: ld      A,B
        ...
        and     01100000b       ; filter only D5&D6 bits
...
STRPAR: ld      A,(SIOBFR+3)    ; load WR4 setting
        and     11110000b       ; reset STOP & PARITY bits
