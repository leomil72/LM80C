...
initVDP:        push    DE              ; store E
                ...
                ; otherwise, it must be $00 so we assume that it's text mode
...
G1MD:           ld      L,B
                ...
                dec     C               ; VDP data mode
LDCLRTBMD1:     out     (C),A           ; after the first byte, the VDP autoincrements VRAM pointer
                nop 
                nop
...
RPT101:         ld      D,$40           ; 64 chars each band
                ld      A,(HL)          ; load data
RPT102:         out     (C),A           ; send it to VRAM
                nop                     ; little delay
                dec     E               ; decrement counter
                jr      NZ,RPT102       ; repeat until zero
                ld      E,$08           ; 8 pixels each pattern
                dec     D               ; decrement char band counter
                jr      NZ,RPT102       ; repeat until zero
                inc     HL              ; next pattern
                djnz    RPT101          ; repeat until covered every band
                ; set pattern table
                ld      HL,$0800        ; address of first cell of 2nd area of pattern table
                ld      DE,LM80CLOGO    ; pointer to logo pattern
                ld      B,L             ; 256 bytes, 8 rows
...
RPT104:         call    LOADLOGOCHRS
                inc     DE              ; next logo pattern
                djnz    RPT104
                ; show logo/message, play a beep, check for CTRL pressing (to disable DOS), and wait a while
                ld      B,$02           ; two times
                xor     A               ; reset A
                ld      D,A             ; 256 times
                ld      E,A             ; x 256 times
                ...
                cp      'Y'
                jr      Z,DEC_D
                ld      A,$01           ; by default, I/O DOS buffer is enabled
...
DEC_D:          nop                     ; does nothing...
                ...
                dec     D
                jr      NZ,DEC_D        ; repeat
                ld      A,(TMPBFR1)     ; sound state
                cp      $02             ; check if sound is to be set off
                call    NZ,BEEPOFF      ; yes
                call    CHKSPCK         ; check if special key (CTRL) has been pressed on keyboard
                                        ; on LM80C 64K, you can disabled the I/O DOS buffer
                djnz    DEC_D           ; repeat
...
; while showing the logo, check if special key (CTRL) is being pressed
; if yes, then disable DOS functions recovering RAM
CHKSPCK:        push    AF
                ld      A,%11111110     ; select CTRL row
                call    READKBLN        ; read row
                bit     3,A             ; test for RUN/STOP
                jp      Z,DOCOLDRESET   ; yes, do a cold reset
                bit     2,A             ; test if CTRL key is pressed
                jr      NZ,LVCKSPLK     ; no, leave
                xor     A               ; yes, so...
                ld      (DOS_EN),A      ; ...disable DOS
                ld      (basicStarted),A; reset BASIC warm start
LVCKSPLK:       pop     AF
                ret
DOCOLDRESET:    xor     A               ; reset A
                ld      (basicStarted),A; reset BASIC warm start
                jp      $0000           ; restart
...
STARTEMPTY:     dec     C               ; VDP address for passing data
...
; set an address into VRAM: address is in HL - HL is changed after it
...
READ_VIDEO_LOC: push    BC              ; store BC
                ...
                out     (C),B           ; high byte
                dec     C               ; VDP data mode
                nop                     ; added to compensate shorter instruction
                nop                     ; wait...
                nop                     ; ...a while
                nop
...
;-------------------------------------------------------------------------------
; char table for jumps
CHRTBL:         defb    HOME
                defw    ATHOME          ; move the cursor to 0,0
                
                defb    CS
                defw    CLEARVIDBUF     ; clear video buffer and position cursor at 0,0

                defb    CR
                defw    CRGRETURN       ; go to the beginning of the next line

                defb    BKSP
                defw    BACKSPACE       ; move cursor left 1 position

                defb    CRSLFT
                defw    CURSORLEFT      ; move cursor left

                defb    CRSUP
                defw    CURSORUP        ; move cursor up

                defb    CRSRGT
                defw    CURSORRIGHT     ; move cursor right

                defb    CRSDN
                defw    CURSORDOWN      ; move cursor up

                defb    INSRT
                defw    INSERTKEY       ; insert a space

                defb    LF
                defw    PLACEHOLDER     ; CURRENTLY WE DON'T PRINT LINE FEED
...
CHAR2VID:       push    AF              ; store AF
                ...
                ld      B,$0A           ; 10 chars to check
                ld      HL,CHRTBL       ; address of key table
RPTCPCK:        cp      (HL)            ; compare with char
                inc     HL              ; beginning of sub-routine address
                jr      NZ,NXTCPCK      ; different, go to next char
                ld      E,(HL)          ; load addres into DE
                inc     HL
                ld      D,(HL)
                ex      DE,HL           ; move user routine's address into HL
                ld      DE,EXITCHAR2VID ; set point of return after the user routine
                push    DE              ; store into stack
                jp      (HL)            ; call user routine - then, the CPU will return (jump) to EXITCHAR2VID
NXTCPCK:        inc     HL              ; jump over 2 cells...
                inc     HL              ; ...to the next char code
                djnz    RPTCPCK         ; repeat
                ; it'a not a special char, just print it
...
SETCSRCOORDS:   ld      (SCR_CURS_X),A  ; store current cursor X
                ...
                ld      A,(TMRCNT)      ; load status of cursor flashing
...
EXITCHAR2VID:   xor     A               ; reset char
                ...
                pop     BC              ; restore BC
                pop     AF              ; restore AF
PLACEHOLDER:    ret                     ; return to caller
...
FLASHCURSOR:    ld      A,(CRSR_STATE)  ; now, check the cursor
                ...
                cp      (HL)            ; compare current state with last state
...
; insert an empty space at the current position of the cursor, moving the following text
; 1 cell straight
INSERTKEY:      call    RSTCHRCRS       ; restore char under the cursor
                call    LOAD_CRSR_POS   ; retrieve address of cursor cell
                ld      (CUR_POS),HL    ; store it
                call    ENDOFLN         ; find address of first free cell after the end of the text from the current cursor position - address into HL -
                                        ; DE is the address of the bottom right cell of the screen
                ld      (ENDTXT),HL     ; store it
                ld      BC,(CUR_POS)    ; load starting address
                xor     A
                sbc     HL,BC           ; how many positions to move?
                jp      Z,MOVSHOWCRS    ; none - leave and re-place cursor
                ld      BC,HL           ; number of chars to scroll into BC
                ld      HL,(ENDTXT)     ; load address of the end of text
CHKHL:          dec     HL              ; decrement to find the address of the char to move
                push    HL
                call    CMP16           ; is it the last cell (bottom right) of screen?
                pop     HL
                jp      NC,NXTINST      ; yes, so jump over - nothing to do
                call    READ_VIDEO_LOC  ; no, so read current char
                inc     HL              ; next cell
                call    WRITE_VIDEO_LOC ; write into new position
                dec     HL              ; decrement to old position
NXTINST:        dec     BC              ; decrement number of chars to move
                ld      A,C
                or      B               ; finished?
...
BACKSPACE:      call    RSTCHRCRS       ; restore char
                ld      HL,(SCR_NAM_TB) ; address of origin of screen (location 0,0)
                ld      BC,HL           ; store into BC
                call    LOAD_CRSR_POS   ; load address of current cursor pos.
                ld      DE,HL           ; copy into DE
                xor     A               ; reset Carry
                sbc     HL,BC           ; check how many chars between
                jp      Z,LVBKSP        ; none, so we are at the top left corner
                ld      (CUR_POS),DE    ; store current cursor position
                call    ENDOFLN         ; check end of text
                ld      DE,(CUR_POS)    ; retrieve cursor position
                xor     A               ; reset Carry
                sbc     HL,DE           ; check if none follows
                jp      Z,LVBKSP        ; no chars follow, so simply move the cursor
                ld      BC,HL           ; save numbers of chars to move
                ex      DE,HL           ; copy starting position into HL
MVBKSP:         call    READ_VIDEO_LOC  ; read char
                dec     HL              ; 1 position to left
                call    WRITE_VIDEO_LOC ; write char
                inc     HL              ; goto next char to copy (2 steps to right)
                inc     HL
                dec     BC              ; decrement # of chars
                ld      A,C
                or      B               ; 0 chars?
                jr      NZ,MVBKSP       ; not finished, continue
                dec     HL
                xor     A
                call    WRITE_VIDEO_LOC ; reset last char
LVBKSP:         call    MVCRS2LFT       ; move cursor to new position
                call    NEWCRSRCOORD    ; set new cursor's coordinates
...
CURSOR_ON:      push    AF              ; store AF
                ...
                or      A               ; is it on?
                jr      NZ,EXITCURSOR_ON; yes, so nothing to do
...
CNTNULL:        ld      (SRTTXT),HL     ; store beginning of text
                call    ENDOFLN         ; find end of text line looking at the end of chars after the cursor position
                ld      (ENDTXT),HL     ; store ending of text line
...
; destroys A, DE, and HL - store address of last char of text line into HL,
; while DE contains the address of the bottom right cell of the screen
ENDOFLN:        ld      A,(SCR_SIZE_H)
...
RPTNLLSRC2:     push    HL
                ...
                pop     HL
                ret     Z               ; if yes, exit because these is nothing after
