...
PRNTCHAR:       call LOAD_CRSR_POS  ; recover position of cursor
                ...
                call SCROLLSCREEN   ; scroll screen up 1 row
                dec e               ; decrement 1 row
SETCRSRY:       xor a               ; then move to cursor X (go to beginning of line)
...
SCROLLSCREEN:   push af             ; store A
                push bc             ; store BC
                push de             ; store DE
                push hl             ; store HL
                ld de,(SCR_NAM_TB)  ; start of the name table - DE will contain the address of the destination row
                ld hl,de            ; load HL with name table address
                ld a,(SCR_SIZE_W)   ; load the screen width
                ld c,a              ; move width into C
                ld b,$00            ; reset B
                add hl,bc           ; HL will contain the address of the source row
                set $06,d           ; set bit 7 to 1 for VRAM writing
                exx                 ; now we use shadow registers for counting
                ld a,(SCR_SIZE_H)   ; reload the screen height
...
GO2NXTROW:      ld d,e              ; D' will count the cols to be moved
LDCHR2MV:       exx                 ; back to main registers for addressing
                ld c,VDP_SET        ; VDP setting mode
                out (c),l           ; low byte of source
                out (c),h           ; high byte of source
                in a,(VDP_DAT)      ; load char
                nop
                out (c),e           ; low byte of dest.
                out (c),d           ; high byte of dest.
                out (VDP_DAT),a     ; send char
                inc hl              ; next source char
                inc de              ; next dest. char
                exx                 ; recover counter registers pairs
                dec d               ; next col
                jr nz,LDCHR2MV      ; no, repeat for the whole row
                djnz GO2NXTROW      ; repeat for the entire screen
                ld a,(SCR_SIZE_W)   ; reload screen width
                ld b,a              ; cells to empty into B
                ld a,$20            ; empty char (space)
                ld c,VDP_SET        ; VDP setting mode
                ld hl,(SCR_NAM_TB)  ; load address of name table
                ld de,(SCR_SIZE_B)  ; load size of screen
                add hl,de           ; start from the last cell
                set $06,h           ; fro writing
RPTEMPTYROW     dec hl              ; last cell onto screen
                out (c),l           ; low byte
                out (c),h           ; then high byte
                out (VDP_DAT),a     ; empty cell
